...
public final class AbstractState implements java.io.Serializable {
  ...
  public AbstractState(Object obj, Class<?> type, boolean init) {
    PalusUtil.checkNull(type, "The declared object type for AbstractState constructor "
        + " can not be null!");
    ...
    //check if the runtime type is a dynamically generated class or not
    Class<?> typeForUse = obj.getClass();
    if(PalusOptions.avoid_serialize_dynamic_class) {
      if(obj.getClass().getName().indexOf("$$") !=  -1) {
        typeForUse = type;
        Log.log("Ignore dynamically-generated class: " + obj.getClass().getName());
      }
    }
    this.clz = typeForUse;
    ...
    if(!PalusUtil.isPrimitive(type) && !type.isPrimitive() && !type.isArray()) {
        PalusUtil.checkTrue(type.isAssignableFrom(this.clz), "Type: " + type.getName()
           + " is not an assignable form of: " + this.clz.getName());
    } else if (type.isArray()) {
        PalusUtil.checkTrue(PalusUtil.isTwoArrayCompatible(type, this.clz), "Two array "
            + "types are not compatible: " + type.getName() + ", " + this.clz.getName());
    }
    ...
    if(this.clz.isPrimitive() || PalusUtil.isPrimitive(this.clz)
      ...
      Field[] allMutableClassFields = this.getNonStaticNonFinalFields(typeForUse/*obj.getClass()*/);
  ...
  public static AbstractState deserialize(ObjectInputStream ois) throws IOException, ClassNotFoundException {
    ...
    PalusUtil.checkTrue(obj instanceof AbstractState, "Object deserialized is not "
        + "an AbstractState type. It is: " + obj);
  ...
  private void saveFieldStates() {
    for(int i = 0; i < this.fields.length; i++) {
      //avoid using toGenericString, since it may include the class name information
      //which is not predictable for dynamically-generated classes, such as classes
      //generated by EasyMock
      this.serializableFields[i] = this.fields[i].getName(); //toGenericString();
      //System.out.println("save field: " + this.serializableFields[i]);
      PalusUtil.checkNull(this.serializableFields[i], "The " + i + "th element"
          + " of serializable fields, when saving field states in AbstractState object.");
    }
  }
  ...
  private void recoverFieldStates() {
    PalusUtil.checkNull(this.clz, "The class type for recovering field states "
        + "can not be null.");
    ...
    for(int i = 0; i < this.serializableFields.length; i++) {
      String fieldName = this.serializableFields[i];
      Log.log("Field name: " + fieldName);
      Field field = null;
      for(Field f : declaredFields) {
        Log.log("  " + f.getName());
        //do not use toGenericString() here, due to the possibility of
        //dynamically generated class files
      ...
      PalusUtil.checkNull(field, "No field is found for: " + fieldName
          + " in class: " + this.clz);
      this.fields[i] = field;
    }
  }
  ...
  public String toString() {
    ...
    PalusUtil.checkTrue(this.fields.length == this.states.length, "The fields length: "
        + this.fields.length + " does not equal to the states length: " + this.states.length);
    ...
    if(this.isPrimtiveOrString) {
      PalusUtil.checkTrue(this.valueOfPrimitiveOrString != null, "The value of primitive "
          + "or string type could not be null.");
